-- Sprint 55.4: Jama visibility — wallet advances, directory scope (depends on 040 enum).

ALTER TABLE transactions
  ALTER COLUMN ledger_id DROP NOT NULL;

ALTER TABLE transactions
  DROP CONSTRAINT IF EXISTS transactions_wallet_advance_requires_contact;

ALTER TABLE transactions
  ADD CONSTRAINT transactions_wallet_advance_requires_contact
  CHECK (
    ledger_id IS NOT NULL
    OR (contact_id IS NOT NULL AND transaction_type = 'wallet_advance')
  );
...
AS $$
BEGIN
  IF NEW.contact_id IS NULL AND NEW.ledger_id IS NOT NULL THEN
    SELECT contact_id
    INTO NEW.contact_id
    FROM ledgers
...
CREATE INDEX IF NOT EXISTS idx_transactions_wallet_advance_logged_at
  ON transactions (logged_at DESC)
  WHERE transaction_type = 'wallet_advance';

DROP FUNCTION IF EXISTS public.get_contact_directory(UUID, INTEGER, INTEGER, UUID, UUID);
...
BEGIN
  ...
  contact_aggregates AS (
    ...
    WHERE c.user_id = p_user_id
    GROUP BY c.id, c.name, c.phone_number, c.wallet_balance
  ),
  ranked_contacts AS (
    SELECT
      ca.contact_id,
  ...
  ORDER BY rc.net_outstanding DESC, rc.wallet_balance DESC, rc.contact_name ASC
...
COMMENT ON FUNCTION public.get_contact_directory(UUID, INTEGER, INTEGER, UUID, UUID) IS
  'Paginated vendor directory scoped to a business workspace. Includes wallet-only contacts (Jama advances with no invoices).';
